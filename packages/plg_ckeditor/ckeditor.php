...
class plgEditorCKeditor extends JPlugin
{
	...
	public function onDisplay($name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null)
	{
		...
		//set style
		if($style_file != '' && file_exists(dirname(__FILE__) . '/styles/' . $style_file))
		{
		...
		//set template
		if($template_file != '' && file_exists(dirname(__FILE__) . "/templates/" . $template_file))
		{
		...
		if($this->params->get('ckfinder', '1') == 1)
		{
			...
			if($access_true && $session->get('CKFinderAccess') && $this->params->get('ckfinder', '0') != 0)
			{
				...
				else
				{
					...
					$saveDir = str_replace('/', DS, $saveDir);
					if(!is_dir(CKFINDER_PATH_BASE . DS . 'media' . DS . 'ckfinder') && !mkdir(CKFINDER_PATH_BASE . DS . 'media' . DS . 'ckfinder', $chmod))
					{
